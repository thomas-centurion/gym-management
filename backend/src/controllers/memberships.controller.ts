  ...
  try {
    ...
    if (error instanceof Error) {
      if (
        ...
        error.message === "El estado no es válido" ||
        error.message ===
...
export const getCurrentMembershipController = async (
  req: Request,
  res: Response
) => {
  try {
    ...
    const membership = await getCurrentMembershipService(
      req.user.id
    );

    if (!membership) {
      return res.status(404).json({
        error: "El socio no tiene una membresía actual",
      });
    }
...
export const changeMembershipPlanController = async (
  req: Request,
  res: Response
) => {
  try {
    ...
    const result = await changeMembershipPlanService(
      req.user.id,
      membershipId,
      plan
    );
    ...
    if (error instanceof Error) {
      if (
        error.message === "El plan es obligatorio" ||
        error.message === "El plan no es válido" ||
        error.message ===
...
export const cancelMembershipController = async (
  ...
  try {
    ...
    const result = await cancelMembershipService(
      req.user.id,
      membershipId
    );
...
export const undoMembershipCancellationController = async (
  req: Request,
  res: Response
) => {
  try {
